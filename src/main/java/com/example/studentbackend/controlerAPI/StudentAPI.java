...
@DeleteMapping("/delete{regnumber}")
    ...
    public ResponseEntity<?>  delete(@PathVariable int regnumber){
        try{
            ...
            return new  ResponseEntity<>("delete Data successfull",HttpStatus.OK);
...
@PutMapping("/byId{regnumber}")

    public ResponseEntity<?> editStudent(@PathVariable int regnumber,@RequestBody Student stud){
        try {
     if (studentRepoo.findById(regnumber).isPresent()){
                stud.setRegnumber(regnumber);
                Student student =studentRepoo.save(stud);
                return  new ResponseEntity<>(student,HttpStatus.OK);
            }else {
                return  new ResponseEntity<>("student areot found",HttpStatus.NOT_FOUND);
            }
        }catch (Exception exception){
            return  new ResponseEntity<>("error",HttpStatus.BAD_REQUEST);
        }
